...
export function hitungPenghematan(voucherData, purchaseAmount = 0, shippingAmount = 0) {
  ...
  if (shippingVoucher.details) {
    let shippingDiscount = shippingVoucher.details.discountValue || 0;

    // Jika shippingAmount 0, shippingDiscount harus 0
    if (shippingAmount === 0) {
      shippingDiscount = 0;
    } else {
      // Berlakukan maksimal shippingDiscount tidak boleh melebihi shippingAmount
      shippingDiscount = Math.min(shippingDiscount, shippingAmount);
    }
