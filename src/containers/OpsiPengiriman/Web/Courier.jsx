...
const Courier = ({
    ...
    return (
        <>
            ...
            <div
                className="flex flex-col relative gap-y-4 pl-3 pb-4"
            >
                <div
                    className="overflow-hidden transition-[max-height] duration-[1200ms] ease-in-out"
                    ref={termsRef}
                    style={{
                        maxHeight: isOpen ? showFullText ? contentHeight : "344px" : "0px",
                    }}
                >
                    {/* {terms} */}
                    {demoDesc}
                </div>
                {isOpen && !showFullText && contentHeight > 344 && (
                    <button
                        onClick={() => setShowFullText(true)}
                        className="text-primary-700 font-medium text-[12px] leading-[14.4px] w-fit"
                    >
                        Lihat selengkapnya
                    </button>
                )}
