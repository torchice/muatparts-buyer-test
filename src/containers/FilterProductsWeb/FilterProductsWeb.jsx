...
function FilterProductsWeb({
    ...
    // tak buat proxy soale tipene gak ke kontrol,🙏🏻
    ...
    return (
        <div className="flex flex-col bg-neutral-50 border border-neutral-400 rounded-md p-4 gap-4 w-[264px] h-fit">
            {/* 25. 03 - QC Plan - Web - Pengecekan Ronda Muatparts - Tahap 2 - LB - 0641*/}
            <span className="text-xl text-[#2c2b2b] font-bold">{t("LabelfilterProdukFilter")}</span>
            <div className="flex flex-col gap-4 w-full divide-y divide-neutral-400">
                ...
                <div className="flex flex-col w-full gap-4 pt-4">
                    ...
                    >
                        ...
                        }} className='font-medium text-xs text-primary-700 select-none cursor-pointer'>{`${t("buttonResetFilter")} ${t('LabelfilterProdukJenisKendaraan')}`}</span>
                    </div>
                </div>
                {/* Jarak */}
                <div className="flex flex-col w-full gap-4 pt-4">
            ...
            <Button Class='w-full max-w-none h-8' color='primary_secondary' onClick={()=>{
                ...
                setMatchFilter([])
                }}>{t("LelangMuatHargaTransportLabelTitleReset")}</Button>
        </div>
    );
}
