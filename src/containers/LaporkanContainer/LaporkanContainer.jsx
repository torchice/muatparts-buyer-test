"use client";

import { useState, useEffect } from "react";
import Button from "@/components/Button/Button";
import Modal from "@/components/AI/Modal";
import { modal } from "@/store/modal";
import Dropdown from "@/components/Dropdown/Dropdown";
import TextArea from "@/components/TextArea/TextArea";
import Input from "@/components/Input/Input";
import Checkbox from "@/components/Checkbox/Checkbox";
import Toast from "@/components/Toast/Toast";
import Bottomsheet from "@/components/Bottomsheet/Bottomsheet";
import toast from "@/store/toast";
import { laporkanProduk } from "@/store/laporkanProduk";
import RadioButton from "@/components/Radio/RadioButton";
import IconComponent from "@/components/IconComponent/IconComponent";
import ImageUploader from "@/components/ImageUploader/ImageUploader";
import SWRHandler from "@/services/useSWRHook";
import NavSelectedMobile from "@/components/Bottomsheet/NavSelectedMobile";
import { X } from "lucide-react";
import axios from "axios";
import ImageComponent from "@/components/ImageComponent/ImageComponent";
import { useHeader } from "@/common/ResponsiveContext";
import { viewport } from "@/store/viewport";
import { useLanguage } from "@/context/LanguageContext";

const api = "v1/";
const baseUrl = `${process.env.NEXT_PUBLIC_GLOBAL_API}v1/`;

export default function Laporkan({
  idProduct = "cade02cd-d60f-4696-ac7e-3943aa",
}) {
  const { isMobile } = viewport();
  const { setModalOpen, setModalContent, setModalConfig } = modal();
  const { useSWRHook, useSWRMutateHook } = SWRHandler();
  const { setShowToast, setDataToast } = toast();
  const { setScreen } = useHeader();

  // Get data opsi di parent saja
  const { data: reportOptions } = useSWRHook(`v1/muatparts/report_option`);

  const { trigger: submitReport } = useSWRMutateHook(
    `${baseUrl}muatparts/report_product`,
    "POST"
  );

  const [opsiList, setOpsiList] = useState([]);

  const { t } = useLanguage();

  useEffect(() => {
    if (reportOptions?.Data) {
      const formattedOptions = reportOptions.Data.map((item) => ({
        name: item.value,
        value: item.id,
      }));
      setOpsiList(formattedOptions);
    }
  }, [reportOptions]);

  const handleSubmitReport = async (isDesktop) => {
    console.log(baseUrl);

    const { validateForm, dataLaporkan, setApiValidationErrors } =
      laporkanProduk.getState();
    const isValid = validateForm({ setShowToast, setDataToast }, isDesktop);
    
    if (isValid) {
      try {
        const photos = dataLaporkan.FotoPelanggaran.value || [];

        const payload = {
          option: dataLaporkan.OpsiPelanggaran.value,
          details: dataLaporkan.DetailPelanggaran.value,
          link: dataLaporkan.LinkPelanggaran.value,
          productID: idProduct,
          photo: photos,
        };

        const data = await submitReport(payload,`${api}muatparts/report_product`);
        if(data) {
          setScreen("");
          setShowToast(true);
          setDataToast({
            type: "success",
            message: "Berhasil melaporkan produk",
          });
          setModalOpen(false);
        }
      } catch (error) {
        if (error?.response?.status === 400 && error?.response?.data?.Data) {
          setApiValidationErrors(error.response.data.Data);
        }
      }
    }
  };

  return isMobile ? (
    <>
      <Toast />
      <LaporkanMobile opsiList={opsiList} onSubmit={handleSubmitReport} t={t} />
    </>
  ) : (
    <>
      <Toast />
      <Modal />
      <Button
        // Class="z-[9999999999999999999999]"
        onClick={() => {
          setModalContent(
            <LaporkanWeb
              opsiList={opsiList}
              onSubmit={handleSubmitReport}
              t={t}
            />
          );
          setModalOpen(true);
          setModalConfig({
            classname: "!w-[472px]",
            withHeader: false,
            withClose: true,
          });
        }}
      >
        Laporkan produk
      </Button>
    </>
  );
}

export const LaporkanWeb = ({ opsiList, onSubmit, onClickCancel }) => {
  const { useSWRMutateHook } = SWRHandler();
  const { setModalOpen } = modal();
  const { setShowToast, setDataToast } = toast();
  const { dataLaporkan, setDataLaporkan, validateForm, resetForm } =
    laporkanProduk();
  const [foto, setFoto] = useState(Array(3).fill(null));
  const [isUploading, setIsUploading] = useState(false);
  const [uploaderKey, setUploaderKey] = useState(0); // Key untuk force re-render

  const { t } = useLanguage();

  const { trigger: uploadImage } = useSWRMutateHook(
    `${process.env.NEXT_PUBLIC_GLOBAL_API}v1/muatparts/upload_report`,
    "POST",
    (url, arg) => {
      return axios({
        url,
        method: "POST",
        data: arg,
        headers: { "Content-Type": "multipart/form-data" },
      });
    }
  );

  const handleUploadImage = (base64String, index) => {
    try {
      setIsUploading(true);

      // Convert base64 to binary
      console.log("ini base", base64String);
      const base64Response = fetch(base64String);
      fetch(base64String)
        .then((res) => res.blob())
        .then((blob) => {
          const formData = new FormData();
          formData.append("file", blob);
          uploadImage(formData).then((response) => {
            if (response?.data?.Data?.url) {
              const newUrl = response.data.Data.url;
              // Get existing photos excluding nulls
              let existingPhotos = foto.filter((f) => f !== null);
              // Remove the same URL if exists to prevent duplicates
              existingPhotos = existingPhotos.filter((f) => f !== newUrl);
              // Insert the new URL to the first position
              existingPhotos.push(newUrl);

              // Keep only first 3 photos
              existingPhotos = existingPhotos.slice(0, 3);

              // Create final array with remaining slots as null
              const finalFotos = [...existingPhotos];
              while (finalFotos.length < 3) {
                finalFotos.push(null);
              }

              // Force complete re-render of all uploaders
              setUploaderKey((prev) => prev + 1);
              setFoto(finalFotos);
              setDataLaporkan("FotoPelanggaran", existingPhotos);
            }
          });
        });
      // const blob = await base64Response.blob();

      //
      // const response = await fetch(`${process.env.NEXT_PUBLIC_GLOBAL_API}v1/muatparts/upload_report`, {
      //   method: 'POST',
      //   body: formData,
      // });

      // if (response?.data?.Data?.url) {
      //   const newUrl = response.data.Data.url;

      //   // Get existing photos excluding nulls
      //   let existingPhotos = foto.filter((f) => f !== null);

      //   // Remove the same URL if exists to prevent duplicates
      //   existingPhotos = existingPhotos.filter((f) => f !== newUrl);

      //   // Insert the new URL to the first position
      //   existingPhotos.push(newUrl);

      //   // Keep only first 3 photos
      //   existingPhotos = existingPhotos.slice(0, 3);

      //   // Create final array with remaining slots as null
      //   const finalFotos = [...existingPhotos];
      //   while (finalFotos.length < 3) {
      //     finalFotos.push(null);
      //   }

      //   // Force complete re-render of all uploaders
      //   setUploaderKey((prev) => prev + 1);
      //   setFoto(finalFotos);
      //   setDataLaporkan("FotoPelanggaran", existingPhotos);
      // }
    } catch (err) {
      console.error("Upload error:", err);
      setShowToast(true);
      setDataToast({
        type: "error",
        message: "Gagal memproses foto",
      });
    } finally {
      setIsUploading(false);
    }
  };

  const handleRemoveImage = (removeIndex) => {
    const newFotos = foto.filter((f, idx) => idx !== removeIndex && f !== null);
    const finalFotos = [...newFotos];
    while (finalFotos.length < 3) {
      finalFotos.push(null);
    }

    // Force complete re-render of all uploaders
    setUploaderKey((prev) => prev + 1);
    setFoto(finalFotos);
    setDataLaporkan("FotoPelanggaran", newFotos);
  };

  useEffect(() => {
    return () => resetForm();
  }, []);

  return (
    <div className="py-8 px-6 pr-[23px]">
      {/* Header tetap */}
      <span className="font-bold text-base text-neutral-900 flex justify-center mb-4 w-[424px]">
        {t("AppMuatpartsLaporkanProdukLabelAppBar")}
      </span>
      {/* Area scroll dengan tinggi tetap */}
      <div className="h-[357px] overflow-y-auto pr-2 mb-4 modal-scroll-area w-[424px]">
        <div className="space-y-4">
          <label className="font-bold text-xs text-neutral-600 block">
            {t("AppMuatpartsLaporkanProdukLabelAppBar")}
          </label>
          <hr className="border-neutral-400" />

          <DivParticleLaporkan
            title={t("AppMuatpartsLaporkanProdukLabelViolationOptions") + "*"}
            hr
            fieldName="OpsiPelanggaran"
          >
            <Dropdown
              classname={`!w-full !min-w-full !max-w-full !border ${
                dataLaporkan.OpsiPelanggaran.validation !== ""
                  ? "!border-error-400"
                  : "!border-neutral-600 hover:!border-primary-700"
              }`}
              placeholder={t(
                "AppMuatpartsLaporkanProdukLabelViolationPlaceholder"
              )}
              options={opsiList}
              displayKey="name" // tambah ini untuk menentukan key yang ditampilkan
              valueKey="value" // tambah ini untuk value yang akan dipilih
              onSelected={(e) => setDataLaporkan("OpsiPelanggaran", e[0].value)}
            />
            {dataLaporkan.OpsiPelanggaran.validation !== "" && (
              <span className="text-error-400 font-medium text-xs -mt-[10px]">
                {dataLaporkan.OpsiPelanggaran.validation}
              </span>
            )}
          </DivParticleLaporkan>

          <DivParticleLaporkan
            title={t("AppMuatpartsLaporkanProdukLabelViolationDetail") + "*"}
            hr
            fieldName="DetailPelanggaran"
          >
            <TextArea
              maxLength={1000}
              hasCharCount={false}
              status={`${
                dataLaporkan.DetailPelanggaran.validation !== "" && "error"
              }`}
              supportiveText={{
                title: `${
                  dataLaporkan.DetailPelanggaran.validation !== ""
                    ? dataLaporkan.DetailPelanggaran.validation
                    : t("AppMuatpartsLaporkanProdukLabelMinChar")
                }`,
              }}
              classname="!w-full !min-w-full !max-w-full"
              // resize="none"
              placeholder={t(
                "AppMuatpartsLaporkanProdukLabelViolationDescription"
              )}
              value={dataLaporkan.DetailPelanggaran.value}
              changeEvent={(e) =>
                setDataLaporkan("DetailPelanggaran", e.target.value)
              }
            />
          </DivParticleLaporkan>

          <DivParticleLaporkan
            title={t("AppMuatpartsLaporkanProdukLabelPhotoEvidence") + "*"}
            hr
            fieldName="FotoPelanggaran"
          >
            <div className="flex gap-4">
              {foto.map((url, idx) => (
                <div
                  key={`container-${uploaderKey}-${idx}`}
                  className="relative"
                >
                  <div className={`${url ? "hidden" : "block"}`}>
                    <ImageUploader
                      className={`!rounded-[4px] !size-[40px] ${dataLaporkan.FotoPelanggaran.validation ? '!border-error-400' : ''}` }
                      getImage={(e) => handleUploadImage(e, idx)}
                      maxSize={10}
                      uploadText=""
                      isCircle={true}
                      isBig={false}
                      onUpload={() => {}}
                      onError={() => {}}
                      error={false}
                      previewTitle={t("labelUnggahBuktiLaporan")}
                    />
                  </div>
                  {url && (
                    <div className="relative border border-[#868686] border-dashed rounded-[4px]">
                      <img
                        src={url}
                        className="!rounded-[4px] !size-[40px] object-cover"
                        alt={`Foto ${idx + 1}`}
                      />
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleRemoveImage(idx);
                        }}
                        className="absolute top-[1px] right-[3px] rounded-full size-4 flex items-center justify-center z-10 bg-white"
                      >
                        <X color="black" size={12} />
                      </button>
                    </div>
                  )}
                </div>
              ))}
            </div>

            {dataLaporkan.FotoPelanggaran.validation ? (
              <span className="font-medium text-error-400 text-xs">
                {dataLaporkan.FotoPelanggaran.validation}
              </span>
            ) : (
              <span className="font-medium text-xs text-neutral-600">
                {t("AppMuatpartsLaporkanProdukLabelPhotoRequirement")}
              </span>
            )}
          </DivParticleLaporkan>

          <DivParticleLaporkan title={t("labelReferenceLink")} optional hr>
            <Input
              placeholder={t(
                "AppMuatpartsLaporkanProdukLabelReferencePlaceholder"
              )}
              value={dataLaporkan.LinkPelanggaran.value}
              // Tambahkan kondisi error
              status={dataLaporkan.LinkPelanggaran.validation ? "error" : ""}
              // Tambahkan supportive text untuk menampilkan pesan error
              supportiveText={{
                title: dataLaporkan.LinkPelanggaran.validation || "",
                desc: "",
              }}
              changeEvent={(e) =>
                setDataLaporkan("LinkPelanggaran", e.target.value)
              }
            />
          </DivParticleLaporkan>

          <div className="flex items-baseline" data-field="CheckboxPelanggaran">
            <div className="translate-y-1">
              <Checkbox
                label=""
                checked={dataLaporkan.CheckboxPelanggaran.value}
                value={dataLaporkan.CheckboxPelanggaran.value}
                onChange={(e) =>
                  setDataLaporkan("CheckboxPelanggaran", e.checked)
                }
              />
            </div>
            <span className="text-neutral-900 text-xs font-medium">
              {t("AppMuatpartsLaporkanProdukLabelDeclaration")}
            </span>
          </div>
        </div>
      </div>

      {/* Footer tetap */}
      <div className="flex justify-center gap-2 w-[424px]">
        <Button
          color="primary_secondary"
          Class="!max-w-[112px] !max-h-8 !font-semibold pb-2"
          onClick={() => {
            resetForm();
            onClickCancel();
          }}
        >
          {t("AppMuatpartsLaporkanProdukLabelCancel")}
        </Button>
        <Button
          Class="!max-w-[112px] !max-h-8 !font-semibold pb-2"
          onClick={() =>
            validateForm({ setShowToast, setDataToast }, true, t) &&
            onSubmit(true)
          }
        >
          {t("AppMuatpartsLaporkanProdukLabelSubmit")}
        </Button>
      </div>
    </div>
  );
};

export const LaporkanMobile = ({ opsiList, onSubmit, t }) => {
  const { useSWRMutateHook } = SWRHandler();
  const { setScreen } = useHeader();
  const {
    setShowToast,
    setDataToast,
    setShowBottomsheet,
    setTitleBottomsheet,
    setDataBottomsheet,
  } = toast();
  const { setModalOpen } = modal();
  const { dataLaporkan, setDataLaporkan, validateForm, resetForm } =
    laporkanProduk();
  const [tempOpsi, setTempOpsi] = useState("");
  const [foto, setFoto] = useState(Array(3).fill(null));
  const [isUploading, setIsUploading] = useState(false);
  const [uploaderKey, setUploaderKey] = useState(0);

  const { trigger: uploadImage } = useSWRMutateHook(
    `${process.env.NEXT_PUBLIC_GLOBAL_API}v1/muatparts/upload_report`,
    "POST",
    (url, arg) => {
      return axios({
        url,
        method: "POST",
        data: arg,
        headers: { "Content-Type": "multipart/form-data" },
      });
    }
  );

  const handleUploadImage = async (base64String, idx) => {
    try {
      setIsUploading(true);
      const response = await fetch(base64String);
      const blob = await response.blob();
      const formData = new FormData();
      formData.append("file", blob);

      const uploadResponse = await uploadImage(formData);

      if (uploadResponse?.data?.Data?.url) {
        const newUrl = uploadResponse.data.Data.url;
        let existingPhotos = foto.filter((f) => f !== null);

        existingPhotos = existingPhotos.filter((f) => f !== newUrl);
        existingPhotos.push(newUrl);
        existingPhotos = existingPhotos.slice(0, 3);

        const finalFotos = [...existingPhotos];
        while (finalFotos.length < 3) {
          finalFotos.push(null);
        }

        setUploaderKey((prev) => prev + 1);
        setFoto(finalFotos);
        setDataLaporkan("FotoPelanggaran", existingPhotos);
      }
    } catch (err) {
      console.error("Upload error:", err);
      setShowToast(true);
      setDataToast({
        type: "error",
        message: t("messageUploadPhotoFailed"),
      });
    } finally {
      setIsUploading(false);
    }
  };

  const handleRemoveImage = (removeIndex) => {
    const newFotos = foto.filter((f, idx) => idx !== removeIndex && f !== null);
    const finalFotos = [...newFotos];
    while (finalFotos.length < 3) {
      finalFotos.push(null);
    }

    setUploaderKey((prev) => prev + 1);
    setFoto(finalFotos);
    setDataLaporkan("FotoPelanggaran", newFotos);
  };

  // Handle pilihan radio
  const handleRadioSelect = (key) => {
    setTempOpsi(key.name);
    renderBottomsheetContent(key.value);
  };

  const renderBottomsheetContent = (currentSelection) => {
    setDataBottomsheet(
      <>
        <div className="flex flex-col gap-4 max-h-[397px] overflow-auto">
          {opsiList.map((key, idx) => (
            <div key={idx}>
              <div className="flex items-center justify-between w-full">
                <span className="font-semibold text-sm text-neutral-900">
                  {key.name}
                </span>
                <RadioButton
                  name="radioOpsiPelanggaran"
                  isChecked={currentSelection === key.name}
                  value={key.name}
                  onClick={() => handleRadioSelect(key)}
                />
              </div>
              {idx !== opsiList.length - 1 && (
                <hr className="border-neutral-400 mt-3" />
              )}
            </div>
          ))}
        </div>
        <Button
          Class="!w-full !max-w-full mt-4"
          onClick={() => {
            setDataLaporkan("OpsiPelanggaran", currentSelection);
            setShowBottomsheet(false);
          }}
        >
          {t("buttonApply")}
        </Button>
      </>
    );
  };

  const handleOpenBottomsheet = () => {
    const initialValue = dataLaporkan.OpsiPelanggaran.value;
    setTempOpsi(initialValue);
    setShowBottomsheet(true);
    setTitleBottomsheet(t("AppMuatpartsLaporkanProdukLabelViolationOptions"));
    renderBottomsheetContent(initialValue);
  };

  return (
    <>
      <div className="space-y-6 py-5 px-4">
        <span className="font-bold text-sm text-neutral-900">
          {t("AppMuatpartsLaporkanProdukLabelHeader")}
        </span>
        <DivParticleLaporkan
          title={t("AppMuatpartsLaporkanProdukLabelViolationOptions") + "*"}
        >
          <div
            className={`!w-full !h-8 !border font-semibold text-sm rounded-md px-3 items-center flex justify-between ${
              dataLaporkan.OpsiPelanggaran.value
                ? "!text-neutral-900"
                : "!text-neutral-600"
            } ${
              dataLaporkan.OpsiPelanggaran.validation !== ""
                ? "!border-error-400"
                : "!border-neutral-600"
            }`}
            onClick={handleOpenBottomsheet}
          >
            {tempOpsi ||
              t("AppMuatpartsLaporkanProdukLabelViolationPlaceholder")}
            <IconComponent src="/icons/chevron-down.svg" />
          </div>
          {dataLaporkan.OpsiPelanggaran.validation !== "" && (
            <span className="text-error-400 font-medium text-xs -mt-[10px]">
              {dataLaporkan.OpsiPelanggaran.validation}
            </span>
          )}
        </DivParticleLaporkan>

        <DivParticleLaporkan
          title={t("AppMuatpartsLaporkanProdukLabelViolationDetail") + "*"}
        >
          <TextArea
            hasCharCount={false}
            status={
              dataLaporkan.DetailPelanggaran.validation !== "" ? "error" : ""
            }
            supportiveText={{
              title:
                dataLaporkan.DetailPelanggaran.validation ||
                t("AppMuatpartsLaporkanProdukLabelMinChar"),
            }}
            classname="!w-full !min-w-full !max-w-full"
            resize="none"
            placeholder={t(
              "AppMuatpartsLaporkanProdukLabelViolationDescription"
            )}
            value={dataLaporkan.DetailPelanggaran.value}
            changeEvent={(e) =>
              setDataLaporkan("DetailPelanggaran", e.target.value)
            }
          />
        </DivParticleLaporkan>

        <DivParticleLaporkan
          title={t("AppMuatpartsLaporkanProdukLabelPhotoEvidence") + "*"}
        >
          <div className="flex gap-4">
            {foto.map((url, idx) => (
              <div key={`container-${uploaderKey}-${idx}`} className="relative">
                <div className={url ? "hidden" : "block"}>
                  <ImageUploader
                    key={`uploader-${uploaderKey}-${idx}`}
                    className="!rounded-xl !size-[72px] border-2 border-dashed !border-neutral-400"
                    getImage={(e) => {
                      console.log(e);
                      // handleUploadImage(e, idx);
                    }}
                    maxSize={10}
                    uploadText={t("AppMuatpartsLaporkanProdukLabelUpload")}
                    isCircle={true}
                    onUpload={(e) => {
                      console.log(e, " upl");
                      handleUploadImage(e, idx);
                    }}
                    onError={(e) => {
                      console.log(e, " err");
                    }}
                    error={false}
                    isLoading={isUploading}
                  />
                </div>
                {url && (
                  <div className="relative">
                    <img
                      src={url}
                      className="!rounded-xl !size-[72px] border-2 border-neutral-400 object-cover"
                      alt={`Foto ${idx + 1}`}
                    />
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleRemoveImage(idx);
                      }}
                      className="absolute top-[4px] right-[5px] rounded-full size-4 flex items-center justify-center z-10 bg-white"
                    >
                      <X color="black" size={12} />
                    </button>
                  </div>
                )}
              </div>
            ))}
          </div>

          {dataLaporkan.FotoPelanggaran.validation ? (
            <span className="text-error-400 font-medium text-xs">
              {dataLaporkan.FotoPelanggaran.validation}
            </span>
          ) : (
            <span className="font-medium text-xs text-neutral-600">
              {t("AppMuatpartsLaporkanProdukLabelPhotoRequirement")}
            </span>
          )}
        </DivParticleLaporkan>

        {/* Rest of the component remains the same */}
        <DivParticleLaporkan title={t("labelReferenceLink")} optional>
          <Input
            placeholder={t(
              "AppMuatpartsLaporkanProdukLabelReferencePlaceholder"
            )}
            value={dataLaporkan.LinkPelanggaran.value}
            status={dataLaporkan.LinkPelanggaran.validation ? "error" : ""}
            supportiveText={{
              title: dataLaporkan.LinkPelanggaran.validation || "",
              desc: "",
            }}
            changeEvent={(e) =>
              setDataLaporkan("LinkPelanggaran", e.target.value)
            }
          />
        </DivParticleLaporkan>

        <div className="flex items-baseline gap-1 sm:items-center">
          <div className="translate-y-1 sm:translate-y-0">
            <Checkbox
              label=""
              checked={dataLaporkan.CheckboxPelanggaran.value}
              value={dataLaporkan.CheckboxPelanggaran.value}
              onChange={(e) =>
                setDataLaporkan("CheckboxPelanggaran", e.checked)
              }
            />
          </div>
          <span className="text-neutral-900 text-xs font-medium sm:text-sm sm:font-bold">
            {t("AppMuatpartsLaporkanProdukLabelDeclaration")}
          </span>
        </div>

      </div>
      {/* //LB - 0038, 25.03 */}
      <NavSelectedMobile classname="!left-0 !sticky">
        <div className="flex justify-center gap-2">
          <Button
            color="primary_secondary"
            Class="!max-w-full !w-full !max-h-10 !font-semibold"
            onClick={() => {
              resetForm();
              setModalOpen(false);
              setScreen("");
            }}
          >
            {t("AppMuatpartsLaporkanProdukLabelCancel")}
          </Button>
          <Button
            Class="!max-w-full !w-full !max-h-10 !font-semibold"
            onClick={() =>
              validateForm({ setShowToast, setDataToast }, false, t) &&
              onSubmit(false)
            }
          >
            {t("AppMuatpartsLaporkanProdukLabelSubmit")}
          </Button>
        </div>
      </NavSelectedMobile>
    </>
  );
};

export const DivParticleLaporkan = ({
  title,
  optional,
  hr,
  children,
  classname,
  fieldName,
}) => {
  const { t } = useLanguage();
  return (
    <>
      <div
        className={`flex flex-col gap-4 ${classname}`}
        data-field={fieldName}
      >
        <span className="font-medium text-xs text-neutral-900 sm:font-semibold sm:text-sm">
          {title}{" "}
          {optional && (
            <span className="italic text-neutral-600 sm:font-semibold sm:!not-italic sm:!text-black sm:!text-[10px]">
              ({t("labelOpsionalLaporkanProduk")})
            </span>
          )}
        </span>
        {children}
      </div>
      {hr && <hr className="border-neutral-400" />}
    </>
  );
};
