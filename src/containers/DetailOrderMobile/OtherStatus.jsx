...
function OtherStatus({
  ...
  // 25. 03 - QC Plan - Web - Pengecekan Ronda Muatparts - Tahap 2 - LB - 0775
  return (
    <div className="bg-neutral-200 flex flex-col gap-2 text-neutral-900">
      ...
      >
        <div className="p-4 flex flex-col gap-4 items-center w-[272px]">
          <span className="bold-base">{t("LabelfilterProdukKodePickUp")}</span>
          <span className="medium-sm text-center">
            {t(
              "LabelfilterProdukTunjukkankodeinikepadaPenjualuntukmengambilpesanankamu"
            )}
          </span>
          {/* 25. 19 - WEB - LB - 0117 */}
          <span className="bold-sm !text-[30px] tracking-[11px]">{detailPesanan?.storeOrders?.[0]?.shippingInfo?.trackingNumber}</span>
        </div>
      </ModalComponent>
      ...
      {detailPesanan?.storeOrders?.[0]?.shippingInfo?.shippingType ===
        ...
        <div className="flex flex-col gap-6 py-6 px-4 bg-neutral-50">
          <span className="semi-sm">
            {t("LabelfilterProdukPesananSiapDiambil")}
          </span>
          <span className="medium-xs text-neutral-700">
            {t(
              "LabelfilterProdukKamuwajibmemberikankodepickupsaatmengambilbarang"
            )}
          </span>
          <Button
          ...
          >
            {t("LabelfilterProdukTampilkanKodePickUp")}
          </Button>
        </div>
      )}
      {/* store and products */}
      {detailPesanan?.storeOrders?.map((val, i) => {
        return (
          <div key={i} className="flex flex-col gap-2 bg-neutral-200">
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col ">
                  ...
                  {val?.items?.length > 2 ? (
                    ...
                    >
                      +{val?.items.length - 2}{" "}
                      {t("AppMuatpartsDaftarPesananBuyerProdukLainnya")}
                    </span>
                  ) : (
                    ""
                  )}
            ...
            <div className="py-6 px-4 flex flex-col gap-6 bg-neutral-50">
              <span className="semi-sm">
                {t("LabelfilterProdukInfoPengiriman")}
              </span>
              <div className="flex flex-col gap-4 medium-sm">
                <span className="text-neutral-600">
                  {t("WebKelolaPesananSellerMuatpartsNomorResi")}
                </span>
          ...
          <div className="flex flex-col gap-6">
            <div className="flex justify-between items-center">
              <span className="medium-sm text-neutral-600">
                ...
                {detailPesanan?.storeOrders?.[0]?.items?.length + " "}
                {t("titleProducts")})
              </span>
              <span className="medium-sm">
            ...
            {detailPesanan?.storeOrders[0].voucherUsed?.map((v) => (
              <div className="flex justify-between items-center font-medium sm:text-sm">
                <div className="text-neutral-600">
                  {`${t("AppMuatpartsDashboardSellerVoucher")} ${detailPesanan?.storeOrders[0]?.storeName} (${v?.voucherCode})`}
                </div>
...
paymentMethod}</span>
          ...
          {/* 25. 15 - QC Plan - Web - Imp Voucher muatparts - LB - 0021 */}
          {detailPesanan?.paymentInfo?.voucherUsed?.map((v) => (
            <div className="flex justify-between items-center font-medium sm:text-sm">
              <div className="text-neutral-600">
                {`${t("AppMuatpartsDashboardSellerVoucher")} Muatparts (${v.kodeVoucher})`}
              </div>
              <div className="text-error-400">
                -{numberFormatMoney(+v.amount)}
              </div>
        ...
        <div className="flex justify-between items-center">
          <span className="semi-sm ">{t("LabelfilterProdukTotalTagihan")}</span>
          <span className="semi-sm">
        ...
        {status?.status === "Tiba di Tujuan" && (
          ...
          >
            {t("LabelfilterProdukAjukanKomplain")}
          </Button>
        )}
      </div>
