...
import { MenungguUlasanSkeleton, UlasanCardSkeleton } from "./UlasanSkeleton";
      ...
      <div className="w-full">
        ...
        <div className="rounded-xl shadow-muatmuat">
          <div className="p-6 space-y-4">
            ...
            {
              isLoading && menuZ?.id===1 &&(
                Array(3).fill('').map((_, index)=>(
                  <MenungguUlasanSkeleton key={`skeleton-menunggu-ulasan-${index}`}/>
                ))
              )
            }
            {
              isLoading && menuZ?.id===2 &&(
                Array(3).fill('').map((_, index)=>(
                  <UlasanCardSkeleton key={`skeleton-ulasan-card-${index}`}/>
                ))
              )
            }
