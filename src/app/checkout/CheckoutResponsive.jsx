...
function CheckoutResponsive({
  ...
  const { setSelectedVouchers, submittedVouchers, clearAllShipping, setPrice } =
    useVoucherMuatpartsStore();

  useEffect(() => {
    if (submittedVouchers?.shipping) {
      console.log("clearing shipping voucher");
      const updatedData = getSelectedVoucher.map((item) => ({
        ...item,
        voucherOngkir: null,
      }));

      setSelectedVoucher(updatedData);
    }
  ...
  const totalProductPrice = getProducts.reduce(
  ...
  function handleSelectedVoucher({
    ...
    // hapus voucher muatparts saat menggunakan voucher penjual
    clearAllShipping();
  ...
  function generateDestinationAddress(val) {
    ...
    return {
      // 25. 11 - QC Plan - Web - Ronda Live Mei - LB - 0357
      id: Number(val?.ID),
  ...
  function generateItems(params) {
    return params?.map((val, index, products) => ({
      ...
      quantity: val?.qty,
      // 25. 11 - QC Plan - Web - Ronda Live Mei - LB - 0357
      unitPrice: Number(val?.priceAfterDiscount),
  ...
  function generateShipping(params) {
    ...
    return {
      shippingType: tipe,
      // 25. 11 - QC Plan - Web - Ronda Live Mei - LB - 0357
      shippingItemID: tipe !== "expedition" ? null : val?.id,
      ...
      ?.reduce(
        (sum, item) =>
          item.discountType === "Nominal"
            ...
            : sum +
              Math.min(
                (Number(+item?.discountValue) / 100) * totalProductPrice,
  ...
  function generateEachVoucher(type, value, discountmax, total) {
    const discountMax = +discountmax || 0;

    const voucher_total =
      type === "Nominal"
        ? Number(+value)
        : discountMax
        ? Math.min((Number(+value) / 100) * total, discountMax)
  ...
  const totalSellerPurchaseVouchers = getSelectedVoucher.reduce(
    (total, item, index) => {
      let voucherValue = 0;
      if (item?.voucherProduct) {
        voucherValue += getVoucherValue(item.voucherProduct, totalProductPrice);
      }
  ...
  const totalSellerShippingVouchers = getSelectedVoucher.reduce(
    (total, transaction, index) => {
      let voucherValue = 0;
      if (transaction?.voucherOngkir) {
        voucherValue += getVoucherValue(
          transaction.voucherOngkir,
          getExpedition?.[index]?.expedisi?.buyerCost || 0
        );
      }
      return total + voucherValue;
    },
  ...
  // 25. 15 - QC Plan - Web - Imp Voucher muatparts - LB - 0033
  ...
  const finalOngkir =
    ...
    totalSellerShippingVouchers;

  const appFee = supportingDataCheckoutDetail?.appFee || 0;
  const adminFee = +paymentOption?.method?.fee || 0;
  ...
  console.log("penting", {
    "0 orders": getProducts,
    "0 voucher": getSelectedVoucher,
    "0 shipping": getExpedition,
  ...
  function handlePay() {
    ...
    if (isDropship) {
      ...
      if (!dropshipValues.name || dropshipValues.name.trim() === "") {
        ...
        errors.name = t(
          "LabelcheckoutRingkasanPembayaranNamaPengirimwajibdiisi"
        );
      }
      ...
      if (!dropshipValues.phone || dropshipValues.phone.trim() === "") {
        // 25. 03 - QC Plan - Web - Pengecekan Ronda Muatparts - Tahap 2 - LB - 0734
        errors.phone = t(
          "LabelcheckoutRingkasanPembayaranNomorTeleponwajibdiisi"
        );
      }
    ...
    triggerOrderPayment({
      orders: getProducts?.map((val, index) => ({
        ...
        sellerID: val?.seller?.id,
        // 25. 11 - QC Plan - Web - Ronda Live Mei - LB - 0357
        originLat: String(val?.seller?.originLat),
        originLng: String(val?.seller?.originLng),
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  onClick={async () => {
                    await setTitleBottomsheet(`Pilih Voucher ${val?.seller?.storeName}`)
                    await setDataBottomsheet(
                      <VoucherCheckout
                        key={i}
                        seller_id={val?.seller?.id}
                        seller_name={val?.seller?.name}
                        onSubmit={(rest) => handleSelectedVoucher({ id: i, ...rest })}
                        product_ids={val?.products.map((item) => item.id)}
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="semi-sm text-[#C22716]" name={i}>
                      ...
                      {/* <pre>{JSON.stringify(getSelectedVoucher.some((a) => a?.id === i), null, 2)}</pre> */}
                    </span>
                  </div>
                  <IconComponent
          ...
          <div className="py-5 px-4 my-2 bg-neutral-50 flex flex-col gap-3">
            ...
            {paymentOption?.method?.id ? (
              <div className="flex gap-2 items-center">
                ...
                <span className="medium-xs">{paymentOption?.method?.name}</span>
              </div>
            ) : (
              ""
            )}
          ...
          <div className="flex flex-col p-4 gap-4 bg-neutral-50">
            ...
            <div className="flex flex-col gap-4 mt-6 pb-6 border-b border-neutral-400">
                  ...
                  <Fragment key={i}>
                    {val?.voucherProduct?.uuid && (
                      <div className="flex w-full justify-between items-center">
                        ...
                        <span className="medium-xs text-error-500">
                          -
                          {numberFormatMoney(
                            generateEachVoucher(
                        ...
                        <span className="medium-xs text-error-500">
                          ...
                          {numberFormatMoney(
                            generateEachVoucher(
                              ...
                              getExpedition?.[i]?.expedisi?.buyerCost || 0
                            )
                          )}
                        </span>
                      </div>
              ...
              {/* {getSelectedVoucher.filter(a=>a?.voucherProduct).length>0&&<div className='flex w-full justify-between items-center'>
                ...
                <span className="medium-xs text-error-500">-{numberFormatMoney(generateVoucher())}</span>
              </div>}
              {getSelectedVoucher.filter(a=>a?.voucherProduct).length>0&&<div className='flex w-full justify-between items-center'>
                <span className="medium-xs text-neutral-600">Voucher Ongkir</span>
                <span className="medium-xs text-error-500">-{numberFormatMoney(generateVoucherOngkir())}</span>
              </div>} */}
            </div>
            <div className="pb-6 border-b border-neutral-400 flex flex-col gap-6">
              ...
              <div className="flex w-full justify-between items-center">
                ...
                <span className="medium-xs">{numberFormatMoney(appFee)}</span>
              </div>
