...
const Dropdown = ({
  ...
  const handleSelect = (option) => {
    if (selected.filter((item) => item.value == option.value).length) {
               ...
               <ul className={`${style.listOptions} `}>
                    {searchBy.map((val, index) => (
                         ...
                         >
                              ...
                              {!!selected.filter((a) => a.value === val.value).length && !isMultipleSelected && <span className="w-4 h-4"><IconComponent src={"/icons/check-circle.svg"} /></span>}
                         </li>
                    ))}
                    {optionsOther.length && !search ? (
                         <>
                              <span onClick={onClickTextOther} className={`${style.list} select-none border-y border-neutral-400 cursor-default hover:unset`}>{optionsOtherText ? optionsOtherText : "Opsi Lainnya"}</span>
                    ...
                    {searchBy.length === 0 && <li className="p-2 text-center text-[12px] select-none">{t("LabelfilterProdukDataTidakDitemukan")}</li>}
               </ul>
          );
     }, [searchBy]);
     ...
     return (
          <>
               <div ref={dropdownRef} className={`${style.main} ${classname} ${disabled?style.disabled:''} ${isError && "border !border-red-500"}`} onClick={onClick}>
                    <button id={id} onClick={handleToggle} className={`${style.buttonPlace} ${!selected.length && "!text-neutral-600"} select-none `}>
                         ...
                         <span className="w-4 h-4"><IconComponent src={selectedIconElement ?? "/icons/chevron-down.svg"} color={arrowColor} classname={`${style.chevron} ${isOpen ? style.chevronRotate : ""}`} /></span>
                    </button>

        {isOpen && (
          <div className={style.listContainer+` ${minWidth&&"!w-[200px]"}  ${position==='right' && "!left-auto !right-0"}`}>
            {onSearchValue && (
