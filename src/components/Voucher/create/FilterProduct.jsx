...
export const fetcherOwnedFilter = async () => {
...
const HoverableMenu = ({
	...
	return (
		...
		>
			<div className="flex-1">{capitalizeString(objKey)}</div>
			<img
				loading="lazy"
			...
			/>
			{open && (
				<div className="absolute left-full top-0  group-hover:block">
					<SubFilterProduct
						items={items}
						selectedItems={selectedItems[objKey] ?? []}
						onSelectionChange={selected =>
							handleSelectionChange(objKey, selected)
						}
					/>
				</div>
			)}
...
const FilterProduct = ({ onFilter }) => {
	...
	const { data } = useSWR("owned_filter", fetcherOwnedFilter);

	useEffect(() => {
		if (data) {
			if (data.Data) {
				delete data.Data.status;
				delete data.Data.condition;
