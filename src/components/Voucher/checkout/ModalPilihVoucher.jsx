...
const VoucherCard = ({
	...
	const discountValue = useMemo(() => {
		...
		return `Diskon ${valueStr}${voucher.isUnlimited ? "" : `, maks. potongan ${formatCurrency(voucher.discountMax, true)}`}`;
	}, [voucher]);
	...
	const expiredText = useMemo(() => {
		// 25. 03 - QC Plan - Web - Pengecekan Ronda Muatparts - Tahap 2 - LB - 0347
		// 24. THP 2 - MOD001 - MP - 024 - QC Plan - Web - MuatParts - Voucher Buyer - LB 0045
...
// 25 03 - QC PLAN -WEB - PENGECEKAN RONDA MUATPARTS - TAHAP 2
	...
	const handleUseVoucher = voucher => {
		if (userTransaction < voucher.transactionMin) {
			setDataToast({
				type: 'error',
				message : `Transaksi minimal untuk menggunakan voucher ini adalah ${formatCurrency(
			...
			// 25. 03 - QC Plan - Web - Pengecekan Ronda Muatparts - Tahap 2 - LB - 0347
			// 24. THP 2 - MOD001 - MP - 024 - QC Plan - Web - MuatParts - Voucher Buyer - LB 0045
			setVoucherError({id:voucher.id,message:`Minimal Transaksi ${formatCurrency(
					voucher.transactionMin,
					true
			)}`})
			return
		...
		// 24. THP 2 - MOD001 - MP - 024 - QC Plan - Web - MuatParts - Voucher Buyer - LB 0045
		if (isNoQuotaLeft){
			setVoucherError({id:voucher.id,message:`Kuota Voucher sudah habis`})
			// 24. MP - 024 - Web - LB - 0045
			return;
		}
		...
		>
			...
			${
				isUsedVoucher
					...
					:
				voucherError
					? "border-red-700 after:border-red-700 before:border-red-700"
					 : 'border-neutral-200 after:border-neutral-200 before:border-neutral-200'
			}
					`}
			/>
			<div className="flex mt-3 px-4 pt-2 pb-3 w-full">
				<div
					className={`text-xs font-medium ${
						// 25. 03 - QC Plan - Web - Pengecekan Ronda Muatparts - Tahap 2 - LB - 0347
						// 24. THP 2 - MOD001 - MP - 024 - QC Plan - Web - MuatParts - Voucher Buyer - LB 0045
						isLessThenAWeek || isLessThanADay
							? "text-red-700"
							: "text-neutral-500"
					}`}
				>
				...
				{((!isNoQuotaLeft || !isExpired) && !isUpcoming) && <button
					className="text-xs font-bold ml-auto text-right text-blue-600"
					onClick={() => handleUseVoucher(voucher)}
				>
					{isUsedVoucher ? "Dipakai" : "Pakai"}
				</button>}
			</div>
		</div>
		{/* 25. 03 - QC Plan - Web - Pengecekan Ronda Muatparts - Tahap 2 - LB - 0347 */}
		{/* 24. THP 2 - MOD001 - MP - 024 - QC Plan - Web - MuatParts - Voucher Buyer - LB - 0045 */}
		<p className="text-red-500 text-xs font-medium mt-2">{voucherError?.id===voucher.id && voucherError?.message}</p>
		</div>
	);
};
const ModalPilihVoucher = ({
	...
	sellerId,
	setUsedVoucher,
	// 24. THP 2 - MOD001 - MP - 024 - QC Plan - Web - MuatParts - Voucher Buyer - LB - 0041
	productIds,
	usedVoucher,
	usedTransaction,
	setUsedTransaction,
	...
	const handleUseTransaction = voucher => {
		setUsedTransaction(voucher);
	};
// 25 03 - QC PLAN -WEB - PENGECEKAN RONDA MUATPARTS - TAHAP 2
	// LB 0318
	const discountValue = useMemo(() => {
		if (!usedVoucher && !usedTransaction) return formatCurrency(0, true);

		let total = 0;

		if (usedVoucher?.discountValue && usedVoucher.discountType === "Nominal")
			total += Number(usedVoucher.discountValue);

		if (usedTransaction?.discountValue && usedTransaction.discountType === "Nominal")
			total += Number(usedTransaction.discountValue);

		if (usedVoucher?.discountValue && usedVoucher.discountType === "Persentase")
			total += (Number(usedVoucher.discountValue) / 100) * totalUserTransaction;

		if (usedTransaction?.discountValue && usedTransaction.discountType === "Persentase")
			total += (Number(usedTransaction.discountValue) / 100) * totalUserTransaction;
		

		return formatCurrency(total, true);
	}, [usedVoucher, usedTransaction]);
	// 25 03 - QC PLAN -WEB - PENGECEKAN RONDA MUATPARTS - TAHAP 2
	//LB 0213
	...
	return (
		<>
			...
			>
				...
				/>
				<div className="relative rounded-xl min-w-[472px] max-w-screen-sm w-full">
					<div className=" relative flex flex-col items-center w-full bg-white rounded-xl  py-8 gap-4">
						<button
						...
						<h1 className="font-bold text-lg">Pilih Voucher {sellerName}</h1>
						<div className="px-6 w-full space-y-4 max-h-[500px] overflow-auto">
							...
							<div className="text-sm text-neutral-600 self-start">
								...
								{tab === 1 &&
									voucherOngkir?.map((voucher, index) => (
										<VoucherCard
											...
											userTransaction={totalUserTransaction}
										/>
									))}
								{tab === 2 &&
									voucherProduct?.map((voucher, index) => (
										<VoucherCard
											...
											onClickDetail={handleClickDetail}
											voucher={voucher}
											onUseVoucher={handleUseTransaction}
											isUsedVoucher={
												usedTransaction?.uuid ===
												voucher?.uuid
											}
											userTransaction={totalUserTransaction}
										/>
						...
						<div className="flex items-center w-full border-t pt-4 px-6">
							<div>
							...
							<button
								className="ml-auto px-4 py-2 bg-primary-700 text-white rounded-full"
								onClick={() => onSubmit()}
							>
								Pakai Voucher
							</button>
