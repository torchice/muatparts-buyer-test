...
function ModalComponent({
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (parentRef.current && !parentRef.current.contains(event.target) && !preventAreaClose) {
        setIsOpen(false);
        setClose();
      }
    };
  ...
  if (getIsOpen)
    return (
      <div
        ...
        className={`${style.main} ${isOpen?style.body:''} w-full overflow-hidden z-[92] flex justify-center items-center ${classname}`}
        onClick={() => {
          if (!preventAreaClose) {
            setIsOpen(false);
            setClose?.();
      ...
      >
        ...
        />
        {
          ...
          <>
            ...
            {children ? (
              <div
                className={`${bgTransparent?'bg-transparent':'bg-white'} rounded-[10px] p-2 z-40 relative min-w-[260px]  min-h-[108px] mx-4 ${
                  !hideHeader ? "pt-[70px]" : ""
                } ${classnameContent}`}
                onClick={(e) => e.stopPropagation()}
              >
                {!hideHeader && (
                  <div className="flex w-full h-[70px] absolute left-0 top-0 z-40 rounded-t-xl overflow-hidden">
                    <ImageComponent
                      src={headerSize==='small'?`/icons/header-small.svg`:`/icons/header-big.svg`}
                      width={600}
                      height={208}
                ...
                {hideHeader && showButtonClose ? (
                  <span className="cursor-pointer" onClick={setClose}>
                    {/* 24. THP 2 - MOD001 - MP - 016 - QC Plan - Web - MuatParts - Paket 024 B - Homepage Buyer - LB-0146 */}
                    <IconComponent
                      classname={"absolute z-50 right-4 top-4 " + style.closeBlue}
        ...
        {
          type==='BottomSheet'&&
            <div onClick={e=> e.stopPropagation()} className={`absolute z-40 w-full h-fit bottom-0 left-0 ${bgTransparent?'bg-transparent':'bg-neutral-50'} rounded-t-2xl flex flex-col gap-4 ${classnameContent}`}>
              ...
              <div className="flex w-full relative justify-center px-4">
                ...
                {(typeof onClickReset==='function')&&<span className="text-primary-700 semi-sx absolute right-4 z-40" onClick={onClickReset}>{textReset?textReset:'Reset'}</span>}
                <span className="bold-sm text-[#000000]">{title}</span>
              </div>
              {children}
            </div>
        }
