...
export function formatOrderData(orderData, params = {}) {
  ...
  if (!orderData || !Array.isArray(orderData) || orderData.length === 0) {
    return {
      ...
      shippingMuatparts: params.voucherShipping || 0,
      productMuatparts: params.voucherPurchase || 0,
  ...
  orderData.forEach((order) => {
    ...
    if (order.selectedShipping) {
      // Use buyerCost for calculations as per the document
      result.totalShippingCost += order.selectedShipping.buyerCost || 0;
      result.shippingCost.push({
        id: order.selectedShipping.id || null,
        cost: order.selectedShipping.buyerCost || 0,
        sellerId: order.seller?.id || null,
      });

      // Calculate insurance cost if used
      if (order.selectedShipping.isUseInsurance) {
        result.totalInsuranceCost += order.selectedShipping.buyerInsurance || 0;
      }
    }
    ...
    if (order.sellerVouchers) {
      ...
      if (
        ...
        order.sellerVouchers.shipping.voucherType === "Biaya Pengiriman"
      ) {
        // Calculate the actual applied amount (min of voucher and shipping cost)
        const voucherValue =
          parseFloat(order.sellerVouchers.shipping.discountValue) || 0;
        ...
        result.shippingVoucher.push({
          name: order.sellerVouchers.shipping.kode || "",
          value: voucherValue,
          sellerId: order.seller?.id || null,
      ...
      if (
        ...
        order.sellerVouchers.purchase.voucherType === "Diskon Produk"
      ) {
        result.productVoucher.push({
          name: order.sellerVouchers.purchase.kode || "",
          value: parseFloat(order.sellerVouchers.purchase.discountValue) || 0,
          sellerId: order.seller?.id || null,
          orderId: order.orderId || null,
        });
      }
    ...
    const shippingCost =
      order.selectedShipping?.buyerCost - result.shippingMuatparts || 0;
    const insuranceCost = order.selectedShipping?.isUseInsurance
      ? order.selectedShipping.buyerInsurance || 0
      : 0;

    // Get product voucher value for this order
    const productVoucherValue = order.sellerVouchers?.purchase
      ? parseFloat(order.sellerVouchers.purchase.discountValue) || 0
      : 0;

    // Get shipping voucher value for this order (limited to actual shipping cost)
    const shippingVoucherValue = order.sellerVouchers?.shipping
      ? parseFloat(order.sellerVouchers.shipping.discountValue) || 0
      : 0;
